...
import (
	...
	"github.com/pedro-git-projects/go-mtg/config"
...
func NewApp() *App {
	...
	// ——— LOG EVERY PLAYER ———
	for pi, player := range g.Players {
		pid := player.ID().Index
		life := g.LifeTotals[pid].Value
		log.Printf("PLAYER %d: entity=%d  starting life=%d", pi+1, pid, life)
	}

	// ——— FIND & LOG EACH LIBRARY ZONE ———
	for entID, zone := range g.ZoneTypes {
		if zone.Value != component.Library {
			continue
		}
		owner := g.Owners[entID].Value
		// find which player this is
		var which int
		for i, p := range g.Players {
			if p.ID().Index == owner.Index {
				which = i + 1
			}
		}
		cards := g.Contains[entID].Value
		log.Printf("Library for player %d (zone=%d): %d cards:", which, entID, len(cards))
		for _, cid := range cards {
			name := g.Names[cid.Index].Value
			mana := g.ManaCosts[cid.Index]
			tl := g.TypeLines[cid.Index]
			color := g.ColorIndicators[cid.Index].Value
			log.Printf(
				"  → card ent=%d  %q  mana=%v  types=[%s]  colors=%s",
				cid.Index,
				name,
				mana,
				tl.String(), // you can add a .String() helper on TypeLineComponent
				color.String(),
			)
		}
	}
...
func (app *App) Layout(outsideWidth, outsideHeight int) (screenWidth, screenHeight int) {
	return config.INTERNAL_W, config.INTERNAL_H
}
